...
def greedy_row_rec2(M_input):     # reconstructing wrt. rows instead of columns
    ...
    while len(ISet)>1:
        ...
        for j in Sremaining:
            if np.sum(pivot_vector*M_copy.T[:,j]) > 0 and j != pivot_index:
                cum_vector=np.copy(M_copy.T[:,j])
                print("starting vector found...", sum(M_copy.T[:,j]),sum(pivot_vector),np.sum(pivot_vector*M_copy.T[:,j]))
                break
                  
        for j in Sremaining:
            if np.sum(pivot_vector*M_copy.T[:,j]) > 0 and j != pivot_index:
                cum_vector=cum_vector*M_copy.T[:,j] # intersection of the pivot and the jth column
                
        print(np.sum(cum_vector),np.sum(cum_vector*pivot_vector))
        M_copy.T[:,pivot_index]=cum_vector+pivot_vector
        ISet.remove(pivot_index)
