...
prfp=5
def Reconstruct(input_file,output_file,Algchoice="FPNA",auto_tune=1,overlapp_coeff=0.3,hist_coeff=20,postprocessing=0,fnfp=100,fnc=1,postprcoef=5,post_fp=0.0001,post_fn=0.1,n_proc=7):
    global prfp
    prfp=postprcoef
    q=Queue()
...
#                    oc_current=overlapp_coeff
    ...
    Flog.close()
    postprocess_col(input_file,output_file,pfn=post_fn,pfp=post_fp)   #Column based post processing, seem to give the best result.
#    if postprocessing==1:
...
parser.add_argument("fp_coeff", default=0.0001 ,type=float,nargs="?")
...
#    Reconstruct(args.inputfile,args.outputfile,fnfp=args.fn_fpratio)
